# 智能化矛盾纠纷综合治理系统（控台 + Spring Boot后端）
...
3. 点击智能助手按钮弹出智能助手页面：顶部展示当事人信息与案件摘要；下方左侧展示智能工作流（React Flow（SmoothStep 折线+圆角）连接、节点可点击、从主节点到当前节点路径高亮绿色，样式本地引用 `frontend/vendor/reactflow.css`）；下方右侧为书签样式页签，优先展示智能指引；点击“办理时间轴”切换为竖状时间树展示。
