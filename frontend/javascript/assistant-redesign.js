(function () {
    var TEXT = {
        ...
        feedbackSubmit: '\u63d0\u4ea4',
        feedbackCancel: '\u53d6\u6d88',
        fileEmpty: '\u5f53\u524d\u6682\u65e0\u53ef\u4e0b\u8f7d\u7684\u7b80\u62a5\u6587\u4ef6\uff0c\u6848\u4ef6\u63a8\u9001\u786e\u8ba4\u540e\u4f1a\u5728\u8fd9\u91cc\u751f\u6210\u3002',
        copy: '\u590d\u5236',
        copied: '\u5df2\u590d\u5236',
        like: '\u70b9\u8d5e',
        dislike: '\u4e0d\u8d5e',
    ...
    function resolveCurrentStageName(detail) {
        ...
        if (status === STATUS.archived) {
            return PROCESS_STEPS[3];
        }
        if (status === STATUS.success) {
            return PROCESS_STEPS[2];
        }
        if (status === STATUS.mediating) {
            return PROCESS_STEPS[1];
        }
    ...
    function applyStaticText() {
        var mapping = {
            ...
            assistantCaseDetailBtn: TEXT.detailBtn,
            assistantCaseAnalysisBtn: TEXT.analysisBtn,
    ...
    function renderProcessBar(detail) {
        ...
        var pieces = [];
        for (var i = 0; i < PROCESS_STEPS.length; i += 1) {
            var stepIndex = i + 1;
            var stepClasses = [];
            ...
            pieces.push('<div class="assistant-step ' + stepClasses.join(' ') + '">' + escapeHtml(PROCESS_STEPS[i]) + '</div>');
    ...
    function buildInfoRows(detail) {
        return [
            ...
            [
                { label: '客户情感分析', value: parseEmotionLabel(detail) }
            ]
        ];
    }
    ...
    function closeCaseMaterial() {
        var modal = document.getElementById('caseMaterialModal');
        if (modal) {
    ...
    function showCaseMaterial(data) {
        var modal = document.getElementById('caseMaterialModal');
        var contentBox = document.getElementById('caseMaterialContent');
        var closeBtn = document.getElementById('closeCaseMaterialBtn');
        if (!modal || !contentBox) {
            return;
        }

        var safeData = data || {};
        var rawMaterial = safeData.caseText || safeData.materialText || safeData.rawMaterial;
        var renderGrid = function (items) {
        ...
        try {
            ...
            contentBox.innerHTML = ''
                + '<section class="case-detail-section case-detail-raw">'
                + '<h4>案件详情</h4>'
                + '<div class="case-detail-text">' + escapeHtml(formatDetailValue(rawMaterial || safeData.parseError || '暂无案件详情')) + '</div>'
                + '</section>';
        }
    ...
    function renderInfoSummary(detail) {
        ...
        root.innerHTML = ''
            ...
            + '<div class="assistant-info-overview-label">案件分类</div>'
            + '<div class="assistant-info-overview-value">' + escapeHtml(textValue(rows[0][0].value)) + '</div>'
            + '</section>'
            + '<section class="assistant-info-overview-item">'
            + '<div class="assistant-info-overview-label">二级分类</div>'
            ...
            + '<div class="assistant-info-overview-label">情感分析</div>'
    ...
    function buildTrackingEvents(detail) {
        ...
        var events = [
            ...
            { id: 'supervise', name: '案件监督', summary: detail ? '监督状态：' + textValue(detail.mediationStatus || detail.handlingProgress) + '；智能建议：' + textValue(detail.mediationAdvice || detail.factsSummary) : '', time: detail && (detail.mediationCompletedAt || detail.updatedAt || detail.createdAt), status: normalizeStatus(detail && detail.mediationStatus) },
    ...
    function buildFileItems(detail) {
        ...
        return [{
            ...
            path: briefingPath,
            department: plainText(detail && detail.recommendedDepartment) || '\u76f8\u5173',
    ...
    function renderFileList(detail) {
        ...
        if (!files.length) {
            root.innerHTML = '<div class="assistant-file-empty"><div class="assistant-file-icon">' + iconSvg('file') + '</div><div class="assistant-file-empty-title">\u6682\u65e0\u7b80\u62a5\u6587\u4ef6</div><div class="assistant-file-empty-text">' + TEXT.fileEmpty + '</div></div>';
            return;
        }
        root.innerHTML = files.map(function (item) {
            return '<button type="button" class="assistant-file-card" data-file-download="' + escapeHtml(item.path) + '">'
                + '<div class="assistant-file-icon">' + iconSvg('download') + '</div>'
                + '<div class="assistant-file-title">\u7b80\u62a5</div>'
                + '</button>';
        }).join('');
    }
    ...
    function renderSideModules(detail) {
        ...
        if (!sideRoot || !tracePanel || !filePanel) {
            return;
        }
        var activeTab = assistantState.sideTab === 'files' ? 'files' : 'trace';
    ...
    function downloadBriefingDocument(pathValue) {
        var safePath = plainText(pathValue);
        if (!safePath) {
            return;
        }
        var link = document.createElement('a');
        link.href = API_BASE + '/recommended-department/briefing-document/download?path=' + encodeURIComponent(safePath);
        link.target = '_blank';
        link.rel = 'noopener';
    ...
    function buildAssistantBriefingIntro(detail) {
        ...
        return '我已成功把案件推送至 ' + department + ' 部门，并生成了案件简报帮助调解人员迅速掌握案件信息。';
    }
    ...
    function resolveAssistantBriefing(detail) {
        ...
        for (var i = 0; i < candidates.length; i += 1) {
            ...
            if (plainText(detail.mediationStatus) === STATUS.mediating) {
    ...
    function shouldTriggerDepartmentPush(detail) {
        var status = plainText(detail && detail.mediationStatus);
    ...
    async function sendChatQuestion(questionText) {
        ...
        if (shouldTriggerDepartmentPush(assistantState.detail)) {
    ...
    function bindEvents() {
        var sendBtn = document.getElementById('assistantChatSendBtn');
        var input = document.getElementById('assistantChatInput');
        var detailBtn = document.getElementById('assistantCaseDetailBtn');
        var analysisBtn = document.getElementById('assistantCaseAnalysisBtn');
        var modal = document.getElementById('assistantModal');
        ...
        if (detailBtn) {
            detailBtn.onclick = function () {
                showCaseMaterial(assistantState.detail || {});
            };
        }
        if (analysisBtn) {
            analysisBtn.onclick = renderCaseAnalysisModal;
        }
        if (modalClose) {
        ...
        if (sideRoot) {
            sideRoot.addEventListener('click', function (event) {
                ...
                if (tabTarget) {
                    assistantState.sideTab = tabTarget.getAttribute('data-side-tab') === 'files' ? 'files' : 'trace';
                ...
                if (fileTarget) {
                    downloadBriefingDocument(fileTarget.getAttribute('data-file-download'));
                }
            });
        }
        if (chatRoot) {
